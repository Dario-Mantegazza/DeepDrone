...
def history_data_plot_crossvalidation(history_list, save_dir):
    ...
    with open(save_dir + "/mean_results.txt", "w+") as outfile:
        ...
        outfile.write("== == == == == == == == == == == ==\n")
        outfile.write("angle_mae:          %.3f" % (np.mean(angle_loss_list)) + '\n')
        outfile.write("angle_mse:          %.3f" % (np.mean(angle_mse_list)) + '\n')
        outfile.write("val_angle_mae:      %.3f" % (np.mean(val_angle_loss_list)) + '\n')
        outfile.write("val_angle_mse:      %.3f" % (np.mean(val_angle_mse_list)) + '\n')
        outfile.write("distance_mae:       %.3f" % (np.mean(distance_loss_list)) + '\n')
        outfile.write("distance_mse:       %.3f" % (np.mean(distance_mse_list)) + '\n')
        outfile.write("val_distance_mae:   %.3f" % (np.mean(val_distance_loss_list)) + '\n')
        outfile.write("val_distance_mse:   %.3f" % (np.mean(val_distance_mse_list)) + '\n')
        outfile.write("height_mae:         %.3f" % (np.mean(height_loss_list)) + '\n')
        outfile.write("height_mse:         %.3f" % (np.mean(height_mse_list)) + '\n')
        outfile.write("val_height_mae:     %.3f" % (np.mean(val_height_loss_list)) + '\n')
        outfile.write("val_height_mse:     %.3f" % (np.mean(val_height_mse_list)) + '\n')
        outfile.write("== == == == == == == == == == == ==\n")
        outfile.close()
    my_dpi = 96
    f_angle = plt.figure(figsize=(1920 / my_dpi, 1080 / my_dpi), dpi=my_dpi)
